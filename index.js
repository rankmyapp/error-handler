...
const errorHandler = ({ blockEvents = [] } = {}) => ({
  middleware(err, req, res, next) {
    ...
    debug(`service:error->${req.protocol + '://' + req.get('host') + req.originalUrl}-> eventId: ${eventId}`)('throws an error %o', err.message);
    return next(err);
  },
  logError(origin, err) {
    ...
    const eventId = Sentry.captureException(err);
    debug(`service:error->${origin}-> eventId: ${eventId}`)('throws an error %o', err.message);
    return true;
  },
});
